...
class InterceptHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        # Replace 0.0.0.0 with 127.0.0.1 in Uvicorn startup messages for clarity
...
async def lifespan(app: FastAPI):
    ...
    if not users:
        ...
        logger.warning("  Login at http://127.0.0.1:8000/admin")
        logger.warning("=" * 60)
...
if __name__ == "__main__":
    ...
    # Display host for logging (show 127.0.0.1 instead of 0.0.0.0 for clarity)
    display_host = "127.0.0.1" if final_host == "0.0.0.0" else final_host
    logger.info(f"Starting Uvicorn server on {display_host}:{final_port}...")
    ...
    uvicorn.run(
        ...
        server_header=False,  # Disable "Server: uvicorn" header
